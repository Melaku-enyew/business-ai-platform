...
export async function saveDataset(dataset) {
  const analysis = {
    ...
    datasetStatus: dataset.datasetStatus ?? dataset.status ?? dataset.pipelineStatus ?? dataset.cleanupStatus ?? 'uploaded',
    version: Number.isFinite(Number(dataset.version)) ? Number(dataset.version) : 1,
...
function rowToDataset(row) {
  ...
  return {
    ...
    module: analysis.module ?? row.module ?? row.moduleName ?? 'dataProcessing',
    workspace: analysis.workspace ?? row.workspace ?? analysis.module ?? 'workspace',
    datasetStatus: analysis.datasetStatus ?? analysis.status ?? row.cleanup_status ?? row.cleanupStatus ?? 'uploaded',
    version: analysis.version ?? 1,
    versionHistory: analysis.versionHistory ?? [],
